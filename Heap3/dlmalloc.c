...
#endif
{
  ...
  if (mem != 0) {
    ...
    */
    ...
    if ((unsigned long)(size) <= (unsigned long)(av->max_fast)  // Size > 80 to go past this if. ~Aaron.
    ...
    */
    ...
    else if (!chunk_is_mmapped(p)) {  // This is where we want to go into. 0x2 is not set in size. ~Aaron.
      nextchunk = chunk_at_offset(p, size);  // Next chunk at p+size.
      nextsize = chunksize(nextchunk);  // Its size is nextchunk+0x4.

      /* consolidate backward */
      if (!prev_inuse(p)) {  // It's trying to consolidate the previous chunk if it's not in use.
        prevsize = p->prev_size;
        size += prevsize;  // Calculating the new size.
        p = chunk_at_offset(p, -((long) prevsize));  /* Treat space at ptr + offset as a chunk */
        unlink(p, bck, fwd);

        /* A copy-paste of the unlink macro.
        #define unlink(P, BK, FD) {                                            \
          FD = P->fd;                                                          \
          BK = P->bk;                                                          \
          FD->bk = BK;                                                         \
